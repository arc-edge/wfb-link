...
- Configure IOUSBHost interface matching, open interface 0, copy descriptor-confirmed bulk pipes, submit bounded bulk-IN and zero-length bulk-OUT smokes, and hold a retained IOUSBHost session across register reads plus both bulk pipe directions on macOS 26.
...
The macOS IOUSBHost fallback now has matching `macos-descriptor-smoke`, `macos-interface-smoke`, `macos-bulk-in-smoke`, `macos-bulk-out-smoke`, `macos-session-smoke`, `macos-bb-smoke`, and `macos-rf-smoke` stages for macOS 26 cases where libusb cannot enumerate the adapter. The descriptor stage is read-only and reports the AWUS036ACH layout as bulk IN `0x81`, bulk OUT `0x02/0x03/0x04`, and interrupt IN `0x85`; the interface, bulk, and session stages prove retained IOUSBHost pipe access; BB/RF stages run the same guarded table programming through default-control transfers.
