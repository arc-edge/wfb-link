...
- Emit JSON diagnostics for current commands, including live `init`, `efuse-dump`, `rx-scan`, `tx-once`, `tx-repeat`, and `bridge-tx-once` reports; `tx-once --dry-run` also builds descriptor-prefixed bytes without touching USB.
