...
- Configure IOUSBHost interface matching, open interface 0, copy descriptor-confirmed bulk pipes, submit a bounded bulk-IN read smoke, and submit a zero-length bulk-OUT smoke on macOS 26.
...
`macos-usb-state`, `macos-descriptor-smoke`, `macos-interface-smoke`, `macos-bulk-in-smoke`, `macos-bulk-out-smoke`, `macos-reg-smoke`, `macos-efuse-dump`, `macos-power-on-smoke`, `macos-firmware-smoke`, `macos-llt-smoke`, `macos-queue-dma-smoke`, and `macos-mac-smoke` are macOS IOUSBHost fallback diagnostics. They are useful on macOS 26 when `usb-probe` cannot see the radio through libusb because IOKit shows the `0x0bda:0x8812` device as `!registered, !matched`, with no interface children until direct configuration. On April 30, 2026, the remote macOS 26 machine at `rownd@rownds-macbook-pro` could open the IOUSBHost device, read descriptors, configure interface matching, copy bulk pipes, complete a zero-length bulk OUT request, issue a bounded bulk IN request, and run default-control register reads plus guarded EFUSE, power-on/RF-reset, firmware, LLT, queue/DMA, MAC/WMAC, BB, and RF setup. Reports include `/tmp/wfb-remote-macos-interface-smoke.json`, `/tmp/wfb-remote-macos-bulk-in-smoke.json`, and `/tmp/wfb-remote-macos-bulk-out-smoke.json`. This proves default-control endpoint access and one-shot pipe IO, not yet the retained full-init RX/TX transport. See `docs/macos-usbhost.md`.
...
The macOS IOUSBHost fallback now has matching `macos-descriptor-smoke`, `macos-interface-smoke`, `macos-bulk-in-smoke`, `macos-bulk-out-smoke`, `macos-bb-smoke`, and `macos-rf-smoke` stages for macOS 26 cases where libusb cannot enumerate the adapter. The descriptor stage is read-only and reports the AWUS036ACH layout as bulk IN `0x81`, bulk OUT `0x02/0x03/0x04`, and interrupt IN `0x85`; the interface and bulk stages prove IOUSBHost pipe access; BB/RF stages run the same guarded table programming through default-control transfers.
