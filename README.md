...
`runtime-flow` is the production-shaped wrapper around the retained full bridge path. It reports adapter identity, endpoints, init readiness, calibration class/evidence source, runtime-owned RX/TX flow telemetry, USB counters, stop reason, and last error without exposing the full diagnostic bridge report shape. It accepts the same bridge runtime controls for now, including `--ready-file` for external orchestration; the remaining migration step is a smaller production binary/API that no longer reuses diagnostic CLI structs.
