...
- Read RTL8812AU registers, EFUSE, guarded power-on/RF-reset writes, firmware download/readiness, LLT programming, and queue/DMA setup through macOS IOUSBHost default-control transfers when macOS has not created libusb-visible interfaces.
...
`macos-usb-state`, `macos-reg-smoke`, `macos-efuse-dump`, `macos-power-on-smoke`, `macos-firmware-smoke`, `macos-llt-smoke`, and `macos-queue-dma-smoke` are macOS IOUSBHost fallback diagnostics. They are useful on macOS 26 when `usb-probe` cannot see the radio through libusb because IOKit shows the `0x0bda:0x8812` device as `!registered, !matched`, with no interface children. On April 30, 2026, the remote macOS 26 machine at `rownd@rownds-macbook-pro` could still open the IOUSBHost device and issue default-control register reads, the guarded EFUSE dump, guarded power-on/RF-reset writes, firmware download/readiness, LLT programming, and queue/DMA setup. Reports: `/tmp/wfb-remote-macos-usb-state.json`, `/tmp/wfb-remote-macos-reg-smoke.json`, `/tmp/wfb-remote-macos-efuse-dump.json`, `/tmp/wfb-remote-macos-power-on-smoke.json`, `/tmp/wfb-remote-macos-firmware-smoke.json`, `/tmp/wfb-remote-macos-llt-smoke.json`, `/tmp/wfb-remote-macos-queue-dma-smoke.json`. This proves default-control endpoint access, not bulk endpoint access or full init. See `docs/macos-usbhost.md`.
