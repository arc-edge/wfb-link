...
- Read RTL8812AU registers and EFUSE through macOS IOUSBHost default-control transfers when macOS has not created libusb-visible interfaces.
...
`macos-usb-state`, `macos-reg-smoke`, and `macos-efuse-dump` are macOS IOUSBHost fallback diagnostics. They are useful on macOS 26 when `usb-probe` cannot see the radio through libusb because IOKit shows the `0x0bda:0x8812` device as `!registered, !matched`, with no interface children. On April 30, 2026, the remote macOS 26 machine at `rownd@rownds-macbook-pro` could still open the IOUSBHost device and issue default-control register reads plus the guarded EFUSE dump. Reports: `/tmp/wfb-remote-macos-usb-state.json`, `/tmp/wfb-remote-macos-reg-smoke.json`, `/tmp/wfb-remote-macos-efuse-dump.json`. This proves control endpoint access, not bulk endpoint access or full init. See `docs/macos-usbhost.md`.
