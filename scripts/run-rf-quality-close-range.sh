...
Configuration is via environment variables. Common overrides:
  ...
  LINK_ID=0x000001           # report/runtime value; Linux WFB CLI gets decimal
...
keys = [
    ...
    "LINK_ID", "WFB_CLI_LINK_ID", "RADIO_PORT", "TX_RATE", "TX_PROFILE", "TX_POWER_MODE",
...
"$SUDO_BIN" -n "$TIMEOUT_BIN" "$RX_SECONDS" \
  "$WFB_RX_BIN" -K "$WFB_KEY" -i "$WFB_CLI_LINK_ID" -p "$RADIO_PORT" -c 127.0.0.1 -u "$LINUX_RX_PORT" "$IFACE" \
...
"$SUDO_BIN" -n "$TIMEOUT_BIN" "$TX_SECONDS" \
  "$WFB_TX_BIN" -d -K "$WFB_KEY" -i "$WFB_CLI_LINK_ID" -p "$RADIO_PORT" -B "$BANDWIDTH_MHZ" -k "$FEC_K" -n "$FEC_N" \
