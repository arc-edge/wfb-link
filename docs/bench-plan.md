...
- The longer 1,000-byte run is sustained packet-flow evidence, not yet a real video/telemetry workload: 92.24 submitted distributor datagrams/s, 100.58 KB/s USB write rate, and 1.32% of one CPU core for the Mac bridge. The next workload step should feed real video/telemetry packets through the same bounded bridge path or add a full-duplex bridge command.
