...
- On macOS 26, the remote hardware Mac saw the RTL8812AU in IOKit as `!registered, !matched` with no interface children, so libusb `usb-probe` could not enumerate or claim it. The IOUSBHost fallback passed descriptor/interface/pipe smokes, register reads, EFUSE, guarded power-on, firmware, LLT, queue/DMA, MAC, BB, RF, integrated `init --macos-usbhost`, bounded `rx-scan --macos-usbhost`, single/repeated descriptor-prefixed TX through `tx-once --macos-usbhost` and `tx-repeat --macos-usbhost`, and one parsed WFB distributor-style datagram through `bridge-tx-once --macos-usbhost`. Reports: `/tmp/wfb-remote-macos-usb-state.json`, `/tmp/wfb-remote-macos-init-usbhost.json`, `/tmp/wfb-remote-macos-rx-scan-usbhost.json`, `/tmp/wfb-remote-macos-tx-once-usbhost.json`, `/tmp/wfb-remote-macos-tx-repeat-usbhost.json`, `/tmp/wfb-remote-macos-bridge-tx-once-usbhost.json`.
