...
- On macOS 26, the remote hardware Mac saw the RTL8812AU in IOKit as `!registered, !matched` with no interface children, so libusb `usb-probe` could not enumerate or claim it. `macos-usb-state`, `macos-reg-smoke`, `macos-efuse-dump`, and `macos-power-on-smoke` passed through IOUSBHost default-control transfers, proving register reads, EFUSE access, and guarded power-on/RF-reset writes but not bulk endpoints. Reports: `/tmp/wfb-remote-macos-usb-state.json`, `/tmp/wfb-remote-macos-reg-smoke.json`, `/tmp/wfb-remote-macos-efuse-dump.json`, `/tmp/wfb-remote-macos-power-on-smoke.json`.
