...
shapes. The current Android bridge wraps an app-owned USB file descriptor with
libusb, so the product app is responsible for USB permission, device open, and
keeping the `UsbDeviceConnection` alive.
