...
- With the adapter back on the local Mac and the receiver reachable as `pi@drone-2f389`, the duplex runner now preflights the Linux peer before claiming the Mac radio and supports `SESSION_ACQUIRE_SETTLE_SECONDS` after observed WFB sessions. The default fast `8/12 MCS1` profile failed L2M at `/tmp/wfb-radio-run-duplex-local-reachable-20260505-100617`, and M2L `5/12` plus L2M `3/12` missed `5/200` M2L payloads at `/tmp/wfb-radio-run-duplex-local-prod-candidate-20260505-100754`. The settled poor-SNR smoke tuple passed at `/tmp/wfb-radio-run-duplex-local-m2l4-l2m3-settle-20260505-101147`: M2L `4/12 MCS1`, L2M `3/12 MCS2`, 20 ms pacing, 100 warmup payloads, 1 s settle, `200/200` both directions, zero decrypt failures, TX `907/907`, zero TX drops/failures, and average Mac RX SNR around 10 dB.
