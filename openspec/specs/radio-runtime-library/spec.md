...
state, TX/RX one-shot execution, live IQC fill application, sweep summaries,
full guarded sweep orchestration/reporting, and TX/RX IQC fill-plan helpers for
calibration callers.
...
#### Scenario: IQK one-shot executed by runtime
- **WHEN** a caller provides RTL8812AU register access and runtime counters for
  the TX or RX IQK one-shot phase
- **THEN** the runtime library drives the upstream trigger/prep, ready polling,
...
#### Scenario: IQK fill applied by runtime
- **WHEN** a caller provides RTL8812AU register access, runtime counters, a
  valid RF path, and mutable TX/RX IQK stage reports
- **THEN** the runtime library applies the selected or fallback TX/RX IQC
  masked BB writes and records the applied fill plans in the stage reports
  without depending on diagnostic command types

#### Scenario: IQK guarded sweep executed by runtime
- **WHEN** a caller provides RTL8812AU register access, runtime counters,
  channel state, and RFE type for guarded runtime IQK
- **THEN** the runtime library owns the bounded sweep loop, setup application,
  TX/RX one-shot execution, IQC fill application, cleanup handling, register
  readback evidence, sweep summaries, and runtime counter deltas without
  depending on diagnostic command types

#### Scenario: Diagnostic command adapts IQK calibration
- **WHEN** the diagnostic runtime-IQK command runs guarded calibration
