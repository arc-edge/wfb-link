...
and TX/RX IQC fill-plan helpers for calibration callers.
...
- **WHEN** the diagnostic runtime-IQK command builds candidate and IQC fill
  reports
- **THEN** it calls the runtime planning, TX/RX one-shot, and outcome-state
  helpers and remains responsible only for sweep orchestration and report
  formatting
