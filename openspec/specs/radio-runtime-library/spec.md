...
state, TX/RX one-shot execution, sweep summaries, and TX/RX IQC fill-plan
helpers for calibration callers.
...
- **THEN** it calls the runtime planning, TX/RX one-shot, and outcome-state
  helpers and remains responsible only for live IQC fill application, sweep
