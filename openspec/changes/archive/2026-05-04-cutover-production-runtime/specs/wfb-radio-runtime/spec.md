...
initializes, receives, and transmits through runtime APIs and runtime-owned
...
- **THEN** it MUST NOT depend on diagnostic command argument structs or
  diagnostic report structs for its production configuration or emitted
  production report

#### Scenario: Production flow rejects diagnostic register experiments
- **WHEN** a caller starts a production runtime flow with diagnostic-only
