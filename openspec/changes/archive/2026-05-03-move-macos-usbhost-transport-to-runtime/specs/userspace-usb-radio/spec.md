...
The system SHALL provide a macOS-only direct-control diagnostic and runtime transport path for RTL8812AU adapters that are visible in IOKit but unavailable through libusb enumeration.
...
- **THEN** the IOUSBHost wrapper, retained session, and bulk/register trait implementations are provided by the runtime library
