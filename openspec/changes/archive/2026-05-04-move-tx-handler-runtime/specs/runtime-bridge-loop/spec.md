...
lifecycle, loop executor, and TX datagram handler that expand TX ingress
sockets, RX forwarding targets, runtime bounds, WFB metadata, scheduling policy,
and queued TX processing without depending on diagnostic command argument
structs.
...
counts, TX datagram metadata, TX submission outcomes, and loop execution
