...
- [ ] 8.4 Run USB probe and init verification on macOS 26
  - Progress: April 30, 2026 remote macOS 26 run on `rownd@rownds-macbook-pro` showed `usb-probe --all` could not enumerate the `0x0bda:0x8812` radio through libusb, while `macos-usb-state` found it in IOKit as `!registered, !matched`, active, 480 Mbps, and without interface children. IOUSBHost direct-control diagnostics passed `macos-reg-smoke`, `macos-efuse-dump`, `macos-power-on-smoke`, and `macos-llt-smoke`, proving default-control endpoint register reads, EFUSE control sequences, guarded power-on/RF-reset writes, and LLT programming but not interface claim, bulk endpoints, or full init. Reports: `/tmp/wfb-remote-macos-usb-state.json`, `/tmp/wfb-remote-macos-reg-smoke.json`, `/tmp/wfb-remote-macos-efuse-dump.json`, `/tmp/wfb-remote-macos-power-on-smoke.json`, `/tmp/wfb-remote-macos-llt-smoke.json`.
