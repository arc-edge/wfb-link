...
- [ ] 8.4 Run USB probe and init verification on macOS 26
  - Progress: April 30, 2026 remote macOS 26 run on `rownd@rownds-macbook-pro` showed `usb-probe --all` could not enumerate the `0x0bda:0x8812` radio through libusb, while `macos-usb-state` found it in IOKit as `!registered, !matched`, active, 480 Mbps, and without interface children before direct configuration. IOUSBHost diagnostics passed `macos-descriptor-smoke`, `macos-interface-smoke`, `macos-bulk-in-smoke`, `macos-bulk-out-smoke`, `macos-session-smoke`, `macos-reg-smoke`, `macos-efuse-dump`, `macos-power-on-smoke`, `macos-firmware-smoke`, `macos-llt-smoke`, `macos-queue-dma-smoke`, `macos-mac-smoke`, `macos-bb-smoke`, and `macos-rf-smoke`, proving descriptor reads, interface matching/opening, pipe copying, one bounded bulk-IN request on `0x81`, one zero-length bulk-OUT request on `0x02`, one retained session across six register reads plus both bulk pipe directions, register reads, EFUSE control sequences, guarded power-on/RF-reset writes, firmware download/readiness, LLT programming, queue/DMA setup, MAC/WMAC setup, BB PHY/AGC setup, and RF radioA/radioB setup. A retained IOUSBHost transport for full init plus RX/TX remains unproven. Reports: `/tmp/wfb-remote-macos-usb-state.json`, `/tmp/wfb-remote-macos-descriptor-smoke.json`, `/tmp/wfb-remote-macos-interface-smoke.json`, `/tmp/wfb-remote-macos-bulk-in-smoke.json`, `/tmp/wfb-remote-macos-bulk-out-smoke.json`, `/tmp/wfb-remote-macos-session-smoke.json`, `/tmp/wfb-remote-macos-reg-smoke.json`, `/tmp/wfb-remote-macos-efuse-dump.json`, `/tmp/wfb-remote-macos-power-on-smoke.json`, `/tmp/wfb-remote-macos-firmware-smoke.json`, `/tmp/wfb-remote-macos-llt-smoke.json`, `/tmp/wfb-remote-macos-queue-dma-smoke.json`, `/tmp/wfb-remote-macos-mac-smoke.json`, `/tmp/wfb-remote-macos-bb-smoke.json`, `/tmp/wfb-remote-macos-rf-smoke.json`.
